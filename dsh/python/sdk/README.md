...
Python subprocess SDK for driving NUAAgent over JSON-RPC stdio. The
runtime inherits normal NUAAgent environment variables such as
...
By default, the SDK launches the bundled single-file `dsh-jsonrpc-agent` executable from the `deepseek-harness-runtime-bin` package and injects that package's default configuration (the stdio JSON-RPC server, agent core, preloaded DeepSeek adapter, JSONL session persistence with an explicitly composed semantic checkpoint policy, local bash) via `DSH_CORDIS_CONFIG`. To run a plugin composition of your own, keep the `@nuaagent/sdk-jsonrpc-server` entry in the config and pass the Cordis config path.
