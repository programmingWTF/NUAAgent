"""Locate the bundled NUAAgent SDK runtime shipped with this package.
...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@nuaagent/sdk-jsonrpc-demo/lib/packaged-bin.js`` on a
...
def resolve_bundled_launch_args(mode: str | None = None) -> tuple[str, ...]:
    ...
    raise ValueError(
        f"unsupported NUAAgent runtime mode {selected!r}: expected 'exe' or 'node' "
