 ...
 * @module @nuaagent/cli/profile-boot
 */
...
import type { EntryOptions } from '@nuaagent/cordis-plugin-loader'
import {
  boot,
  composeEntries,
...
import { DSH_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@nuaagent/launch-environment'
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@nuaagent/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@nuaagent/cordis-plugin-hmr', config: { root: [] } })
      }
