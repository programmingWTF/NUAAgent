...
import { boot, loadOverlayPatches } from '@nuaagent/app-boot'
import SystemPrompt from '@nuaagent/system-prompt'
import ToolRuntime from '@nuaagent/tools'
import * as McpClient from '@nuaagent/mcp-client/src/index.ts'
...
describe('third-party memory MCP example overlays', () => {
  it.each(examples)('parses $file with the documented generic plugin fields', (contract) => {
    ...
    expect(row.name).toBe('@nuaagent/mcp-client')
