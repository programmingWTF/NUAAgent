...
import { startMockLlmServer } from '@nuaagent/llm-mock-server'
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    dsh: { profile: { bundles: ['@nuaagent/base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@nuaagent/cmdline` and `commander` through the profile module
...
function createStartupFixture(): StartupFixture {
  ...
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    ...
    "import { parseCmdline } from '@nuaagent/cmdline'",
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('activates a dependency that gained dsh.bundle in a later update', async () => {
    ...
    try {
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@nuaagent/base'])
      ...
      expect(manifest.dsh.profile.bundles).toEqual(['@nuaagent/base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@nuaagent/agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @nuaagent/base')
      expect(stdout).toContain("name: '@nuaagent/host-webserver'")
    }, 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@nuaagent/headless'")
      expect(stdout).not.toMatch(/name: '@nuaagent\/host-/)
      expect(stdout).not.toContain("name: '@nuaagent/web-app'")
      expect(stdout).not.toMatch(/name: '@nuaagent\/client-/)
    }, 30_000)
