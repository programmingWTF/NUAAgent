...
import { entryListSchema } from '@nuaagent/cordis-plugin-include'
import { evaluate } from '@nuaagent/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@nuaagent/app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@nuaagent/base', '@nuaagent/web-app'])
    ...
    for (const name of ['@nuaagent/pwsh-sandbox', '@nuaagent/tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@nuaagent/base'])
