...
import Include, { type PatchOptions } from '@nuaagent/cordis-plugin-include'
import Group from '@nuaagent/cordis-plugin-group'
import { scrubRequestHeaders, stabilizeFixtureMessageIds } from '@nuaagent/acp-snapshot'
import {
  assertEntriesLoaded,
  composeEntries,
  healProfilesModuleFallback,
  loadOverlayPatches,
} from '@nuaagent/app-boot'
import { dshHomePath } from '@nuaagent/home-paths'
import { settingsNamespace } from '@nuaagent/settings'
import { LlmAdapter } from '@nuaagent/llm'
import type {
  LlmModelInfo, LlmProviderInfo, LlmResolvedModelInfo, StreamChunk,
} from '@nuaagent/llm'
import type { ReplayHandle } from '@nuaagent/llm-replay'
import { installLlmReplay, parseSessionLog } from '@nuaagent/llm-replay'
import SessionStore, {
  packChunkRuns,
  ...
  type SessionEvent,
  type SessionHeader,
} from '@nuaagent/session'
import JsonlSessionPersistence from '@nuaagent/session-persistence-jsonl'
// Empty type imports carry the webServer/agents/sessionPersistence Context merges.
import type {} from '@nuaagent/host-webserver'
import type {} from '@nuaagent/agent'
import { provideCmdline } from '@nuaagent/cmdline'
...
// } from '@nuaagent/client-ui-settings-models'
...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'NUAAgent 目前的 0.1 版本仍处在面向 Harness 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 NUAAgent 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 Harness 开发者加入 DSH 插件生态。',
    continueLabel: '继续',
  },
} as const
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const patches: PatchOptions[] = [
    ...
    // (apps/web IS @nuaagent/web-frontend); only the URL line is silenced.
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@nuaagent/host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@nuaagent/client-ui-directory-picker-browse' },
    ] },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@nuaagent/tool-cordis' },
      ] }]
      : [],
  ...
  try {
    ...
    // `@nuaagent/cordis-plugin-group` by name.
