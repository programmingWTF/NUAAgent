...
`@nuaagent/client-web` import `AppWebEntry`、从
`@nuaagent/client-modules/client` import boot manifest 类型：启动真实 shell 正是该
harness 的用途，且这两个包本来就在 Host 图中。另外，chat 场景从
`@nuaagent/client-runtime/client` import `conversationContextKey`，因为
