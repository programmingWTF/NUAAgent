...
## Do not import `@nuaagent/client-*` here
...
`@nuaagent/goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@nuaagent/client-web` and the boot-manifest
type from `@nuaagent/client-modules/client`: booting the real shell is what
...
`@nuaagent/client-runtime/client` because `client/runtime` is reachable
