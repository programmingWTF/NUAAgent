...
卸载会释放 effect（[第 2 章](02-lifecycle-and-effects.md)），加载则遵循依赖关系（[第 3 章](03-services.md)），因此 HMR 可以先卸载、再加载，以替换正在运行的插件。`@nuaagent/cordis-plugin-hmr` 插件会监视文件，并在保存时执行这一过程。
