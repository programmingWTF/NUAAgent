...
The file-by-file checklist for a new `@nuaagent/<name>` package. This checklist is validated against the bash and adapter packages as templates; if it drifts from them, fix it here.
...
| File | Change |
|---|---|
| `tsconfig.base.json` | no edit for an existing group; for a new group, add a `./packages/<group>/*/src` candidate to the `@nuaagent/*` wildcard |
...
Use `SDK` only for the JSON-RPC client/server protocol used by the supported Python and TypeScript SDKs. NUAAgent itself is an agent harness, not an SDK project. Use the canonical product spelling `Typert`, never `TypeRT` or `typeRT`.
