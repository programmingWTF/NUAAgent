...
NUAAgent is a plugin-based agent harness on vendored Cordis: **everything is a plugin**. Read [docs/architecture.md](docs/architecture.md) before changing `packages/`; follow [docs/AGENTS.md](docs/AGENTS.md) for documentation.
...
packages/    @nuaagent/<pkg> workspaces at packages/<group>/<pkg>/
...
native/      @nuaagent/node-addon-landlock-run source of record (see native/README.md)
...
- Every npm package is `@nuaagent/<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@nuaagent/cordis` is a peerDependency (+ dev) of every harness package.
