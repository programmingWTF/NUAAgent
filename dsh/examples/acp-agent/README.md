...
Stdout carries only newline-delimited ACP JSON-RPC. `@nuaagent/acp-demo` installs no stdout logger; leaf additions must use stderr for diagnostics.

The automation contract — supported methods, baseline prompt content, committed-text output, and the intentionally absent UI surfaces — lives in [`@nuaagent/acp`](../../packages/acp/acp/README.md).
