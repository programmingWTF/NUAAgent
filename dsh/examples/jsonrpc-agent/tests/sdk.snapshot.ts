 ...
 * REAL `@nuaagent/sdk-client`, drives one turn over stdio JSON-RPC,
...
import { resolveExampleLaunch } from '@nuaagent/loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@nuaagent/sdk-client'
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@nuaagent/system-prompt') return []
