...
| 包 | 路径 | ctx 面 | 本期 |
| --- | --- | --- | --- |
| `@nuaagent/storage` | `packages/storage/storage/` | `ctx.storage`（枢纽） | ✓ |
| `@nuaagent/storage-json` | `packages/storage/storage-json/` | 注册后端 `json` | ✓ |
| `@nuaagent/storage-sqlite` | `packages/storage/storage-sqlite/` | 注册后端 `sqlite` | ✓ |
| `@nuaagent/storage-domain` | `packages/storage/storage-domain/` | 挂载 `ctx.storage.domain` | ✓ |
