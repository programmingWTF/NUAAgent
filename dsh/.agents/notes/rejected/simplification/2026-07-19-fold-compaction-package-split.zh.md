...
压缩（compaction）目前拆分在两个包中：`@nuaagent/compaction` 拥有一个含两个方法的抽象服务和共享类型，`@nuaagent/compaction-basic` 拥有唯一的完整提供方。交付配置只加载 basic 包，除了该提供方外，没有生产包独立消费 Service Definition 包。
...
把 basic 实现移入 `@nuaagent/compaction`，并删除 `@nuaagent/compaction-basic`。`ctx.compaction`、`CompactionResult`、共享 transcript（文本记录）和工具配对辅助方法、现有配置以及具体压缩算法都由一个包负责。
...
- 删除 `@nuaagent/compaction-basic` 及其工作区和包元数据。
- `@nuaagent/compaction` 拥有当前配置、插件类、算法、类型、事件和共享辅助方法。
...
这是一项有意实施的预发布包名收缩。加载 `@nuaagent/compaction-basic` 的嵌入方必须切换包，未来的后端替换也需要重新提取边界。只有在仍然只有一个完整实现时，这项代价才可接受；如果第二个后端先行落地，应重新评估是否接纳本提案。
