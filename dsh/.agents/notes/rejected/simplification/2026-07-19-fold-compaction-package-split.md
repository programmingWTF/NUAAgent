...
Move the basic implementation into `@nuaagent/compaction` and remove `@nuaagent/compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@nuaagent/compaction-basic` and its workspace/package metadata are removed.
- `@nuaagent/compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
