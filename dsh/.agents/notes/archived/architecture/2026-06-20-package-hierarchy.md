...
Packages are grouped by modular role at a uniform `packages/<group>/<pkg>/` depth. Group directories are pure containers (no `package.json`); every package keeps its `@nuaagent/<pkg>` name — this is repo structure and maintenance policy, not package renaming.
...
- **A third tier (`adapters/` / `impls/` under each family)** — rejected: uniform depth 2 keeps the workspace glob a clean `packages/*/*` and lets one `@nuaagent/*` tsconfig wildcard resolve every package.
