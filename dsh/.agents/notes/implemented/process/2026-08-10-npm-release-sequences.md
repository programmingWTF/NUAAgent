...
|---|---|---|---|---|
| dsh | `packages/*/*` + `apps/*` (`@nuaagent/cli` and `@nuaagent/web-frontend`) | one version for the family and the workspace root, `0.0.x` | `dsh-v<version>` | `release.yml` |
...
`scripts/check-workspace-constraints.ts` requires the protocol, so a new package cannot reintroduce a hand-written range; the invariant-companion rule requires `workspace:^` for `@nuaagent/invariants` for the same reason.
