...
The existing unscoped npm names are owned by the standalone publisher account rather than the `@nuaagent` organization. Moving only the workflow would therefore leave publication dependent on a personal credential outside the repository's release ownership.
...
- **Give the launcher the root DeepSeek Harness version and publish the complete monorepo recursively** — rejected because this change owns one three-package public family, not the independent `@nuaagent/*` baseline. The [artifact-first npm baseline proposal](../../proposed/process/2026-08-04-artifact-first-npm-baseline-publication.md) explicitly keeps native workspaces outside its target set.
