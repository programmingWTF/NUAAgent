...
`@nuaagent/tool-fs` 在首个文件系统工具套件中暴露以下三个面向模型的工具：
...
schema 使用 snake_case 字段名（`file_path`、`old_string`、`new_string`、`replace_all`），与 Claude Code 及现有 NUAAgent 工具 schema 示例保持一致。消费方包将这些面向模型的名称转换为 `ctx.fs` 调用和 `fs/*` 事件分发。
