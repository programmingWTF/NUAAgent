...
| Package | Role |
|---|---|
| `@nuaagent/subagent` | interface: `SubagentRuntime` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@nuaagent/subagent-spawn-in-process` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@nuaagent/subagent-fork-in-process` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@nuaagent/subagent-acp` | implementation: an ACP client driving a configured child process |
| `@nuaagent/subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@nuaagent/subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@nuaagent/tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |
