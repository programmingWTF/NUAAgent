...
`@nuaagent/tool-fs` exposes these three model-facing tools in the first filesystem suite:
...
The schema uses snake_case field names (`file_path`, `old_string`, `new_string`, `replace_all`) to align with Claude Code and with existing NUAAgent tool-schema examples. The Consumer package translates these model-facing names into `ctx.fs` calls and `fs/*` event dispatches.
