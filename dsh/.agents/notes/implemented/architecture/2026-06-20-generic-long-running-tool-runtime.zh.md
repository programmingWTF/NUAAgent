...
`JobRegistry` 是 `@nuaagent/jobs` 中的 Service Definition；进程内 Service Provider 是 `@nuaagent/jobs-local` 中的 `LocalJobRegistry`（该拆分记录在[任务注册表约定 Agent Note](2026-07-26-job-registry-seam.md)中）。
