...
One resolver owns the harness home, in `@nuaagent/home-paths`, single-root:
...
`@nuaagent/home` is deleted. Its three importers (`dsh-tool-bash`, `dsh-skill-filesystem`, `dsh-agent-spine-demo`) import `resolveDshHome` from `dsh-home-paths`.
