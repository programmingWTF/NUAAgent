...
1. `@nuaagent/web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@nuaagent/web-search-exa`, `@nuaagent/web-search-perplexity`, `@nuaagent/web-search-deepseek`, and `@nuaagent/web-fetch-http`.
3. `@nuaagent/tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@nuaagent/tool-web  --depends on-->  @nuaagent/web  <--depends on--  @nuaagent/web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @nuaagent/web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @nuaagent/web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @nuaagent/web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@nuaagent/web-search-exa"] -->|registerSearchProvider| web["@nuaagent/web / ctx.web"]
  perplexity["@nuaagent/web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@nuaagent/web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@nuaagent/web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@nuaagent/tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@nuaagent/web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@nuaagent/tool-web` depends on `@nuaagent/web`, `@nuaagent/tools`, `@nuaagent/system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@nuaagent/web'
...
- id: web-search-exa
  name: '@nuaagent/web-search-exa'

- id: web-search-perplexity
  name: '@nuaagent/web-search-perplexity'

- id: web-search-deepseek
  name: '@nuaagent/web-search-deepseek'

- id: web-fetch-http
  name: '@nuaagent/web-fetch-http'
