...
| Package | Role |
|---|---|
| `@nuaagent/spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@nuaagent/spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@nuaagent/spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- id: spill-local
  name: '@nuaagent/spill-local'

- id: spill-policy
  name: '@nuaagent/spill-policy'
  config:
...
- `@nuaagent/output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@nuaagent/spill` owns saving final text and returning a locator plus retrieval hint.
- `@nuaagent/spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
