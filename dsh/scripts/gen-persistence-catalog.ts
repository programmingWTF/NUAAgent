...
const SESSION_PACKAGE = '@nuaagent/session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@nuaagent/session/types'
 ...
 * top-level declaration (in `@nuaagent/session`) and any declaration
 * merge inside a `declare module '@nuaagent/session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @nuaagent/session/known-event-types',
    ' */',
    '',
    '/**',
