...
import type { ToolSchema } from '@nuaagent/llm'
import AgentRegistry from '@nuaagent/agent'
import type { Agent } from '@nuaagent/agent'
import { createScope } from '@nuaagent/scope'
import SessionStore, { SessionId } from '@nuaagent/session'
import SessionProjectionRegistry from '@nuaagent/session-projection'
import SqliteSessionQueryEngine from '@nuaagent/session-query-sqlite'
import GoalService from '@nuaagent/goal'
import SystemPrompt from '@nuaagent/system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@nuaagent/tools'
import LocalBashExecutor from '@nuaagent/bash-local'
import * as BashEnvPlugin from '@nuaagent/shell-env'
import { PwshLocalExecutor } from '@nuaagent/pwsh-local'
import LocalSubprocessRuntime from '@nuaagent/subprocess-local'
import LocalFileSystem from '@nuaagent/fs-local'
import { AttachmentStore } from '@nuaagent/attachment'
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@nuaagent/attachment'
import UserQuestionService from '@nuaagent/user-questions'
import PlanModeController from '@nuaagent/plan-mode'
import WebRuntime from '@nuaagent/web'
import * as WebSearchExa from '@nuaagent/web-search-exa'
import * as WebFetchLocal from '@nuaagent/web-fetch-http'
import SubagentRuntime from '@nuaagent/subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@nuaagent/subagent'
import * as ToolSubagentControl from '@nuaagent/tool-subagent-control'
import * as ToolSubagentListAgents from '@nuaagent/tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@nuaagent/tool-subagent-report'
import SkillRegistry from '@nuaagent/skill'
import * as SkillFileSystem from '@nuaagent/skill-filesystem'
import LocalJobRegistry from '@nuaagent/jobs-local'
import * as ToolAskUser from '@nuaagent/tool-ask-user'
import * as ToolBash from '@nuaagent/tool-bash'
import * as ToolPwsh from '@nuaagent/tool-pwsh'
...
import CordisHostRunner from '@nuaagent/cordis-host-runner'
import * as ToolCordis from '@nuaagent/tool-cordis'
import * as ToolFs from '@nuaagent/tool-fs'
import * as ToolFsSearch from '@nuaagent/tool-fs-search'
import * as ToolStrReplaceEditor from '@nuaagent/tool-str-replace-editor'
import TerminalSessionService from '@nuaagent/terminal'
import * as ToolPty from '@nuaagent/tool-terminal'
import * as ToolGoal from '@nuaagent/tool-goal'
import * as ToolSchedule from '@nuaagent/schedule'
import Lsp from '@nuaagent/lsp'
import * as ToolLsp from '@nuaagent/tool-lsp'
import * as ToolSkill from '@nuaagent/tool-skill'
import * as ToolSessionQuery from '@nuaagent/tool-session-query'
import * as ToolTasks from '@nuaagent/tool-jobs'
import type TeamService from '@nuaagent/experimental-agent-team'
import * as ToolTeam from '@nuaagent/experimental-tool-agent-team'
import * as ToolTodo from '@nuaagent/tool-todo'
import * as ToolSubagent from '@nuaagent/tool-subagent'
import * as ToolWeb from '@nuaagent/tool-web'
import VmWorkflowEngine from '@nuaagent/workflow-worker-thread'
import * as ToolRalph from '@nuaagent/tool-ralph'
import * as ToolWorkflow from '@nuaagent/tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@nuaagent/tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@nuaagent/tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@nuaagent/pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `DSH_*` environment comes from `@nuaagent/shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@nuaagent/tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@nuaagent/cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@nuaagent/tool-bash-persistent',
  ...
  {
    pkg: '@nuaagent/tool-pwsh-persistent',
  ...
  {
    pkg: '@nuaagent/tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@nuaagent/fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. The image tool is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@nuaagent/tool-fs-search',
    dir: 'tool-fs-search',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@nuaagent/lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@nuaagent/tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@nuaagent/tool-session-query',
  ...
  {
    pkg: '@nuaagent/tool-subagent-control',
  ...
  {
    pkg: '@nuaagent/tool-subagent-report',
  ...
  {
    pkg: '@nuaagent/experimental-tool-agent-team',
    dir: 'tool-agent-team',
