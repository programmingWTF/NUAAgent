...
describe('tierExternalDeps', () => {
  it('tiers by declaring area, not by the declaring section name', () => {
    const { manifests, names } = workspace({
      ...
      'packages/test-support/loader-smoke/package.json': { name: '@nuaagent/loader-smoke', dependencies: { 'smoke-helper': '^1' } },
      'packages/test-support/client-runtime/package.json': { name: '@nuaagent/client-test-runtime', dependencies: { 'test-lib': '^1' } },
      ...
      'packages/mcp/mcp-client/package.json': { name: '@nuaagent/mcp-client', dependencies: { 'protocol-sdk': '^1' }, devDependencies: { 'protocol-fixture-server': '^1' } },
      'apps/cli/package.json': { name: '@nuaagent/cli', dependencies: { 'cli-lib': '^1', '@nuaagent/mcp-client': 'workspace:^' } },
    })
  ...
  it('keeps a package runtime when any shipping area declares it, and excludes workspace links', () => {
    const { manifests, names } = workspace({
      ...
      'packages/interaction/tui/package.json': { name: '@nuaagent/tui', dependencies: { shared: '^1', '@nuaagent/cli': 'workspace:^' } },
      'apps/cli/package.json': { name: '@nuaagent/cli' },
    })

    expect(tierExternalDeps(manifests, names).get('shared')).toBe(true)
    expect(tierExternalDeps(manifests, names).has('@nuaagent/cli')).toBe(false)
  })
})
...
describe('parseVendoredRows', () => {
  ...
  it('yields nothing when the table columns change, so the generator fails loud', () => {
    expect(parseVendoredRows('| `cordis/` | `@nuaagent/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\n')).toEqual([])
  })
