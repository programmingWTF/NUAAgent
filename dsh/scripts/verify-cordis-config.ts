...
const CHOOSER_PACKAGE = '@nuaagent/host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@nuaagent/host-directory-picker-native',
  '@nuaagent/host-directory-picker-browse',
  '@nuaagent/client-ui-directory-picker-browse',
  '@nuaagent/client-ui-directory-picker-native',
]
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if ((value.group === true || value.name === '@nuaagent/cordis-plugin-group') && isUnknownArray(value.config)) {
  ...
  if (value.name !== '@nuaagent/cordis-plugin-include') return
