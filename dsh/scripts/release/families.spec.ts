...
describe('release families', () => {
  it('excludes private experimental packages from the dsh release', () => {
    ...
    expect(members.map(member => member.name)).not.toContain('@nuaagent/experimental-agent-team')
  })
  ...
  it('bumps private dsh packages without adding release tags', () => {
    ...
    const published = member('packages/core/published', '@nuaagent/published')
  ...
  it('names one tag for the whole dsh family and one per vendored package', () => {
    ...
    const cli = member('apps/cli', '@nuaagent/cli')
    const cordis = { ...member('vendor/cordis', '@nuaagent/cordis'), version: '4.0.1' }
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const dsh = releaseFamily('dsh')
    const members = [member('apps/cli', '@nuaagent/cli'), { ...member('apps/web', '@nuaagent/web-frontend'), version: '0.0.2' }]
  ...
  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@nuaagent/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@nuaagent/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@nuaagent/consumer', { dependencies: { '@nuaagent/library': 'workspace:^' } }),
      member('packages/a/library', '@nuaagent/library'),
      member('packages/a/zebra', '@nuaagent/zebra'),
    ]
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@nuaagent/left', { dependencies: { '@nuaagent/right': 'workspace:^' } }),
      member('packages/a/right', '@nuaagent/right', { dependencies: { '@nuaagent/left': 'workspace:^' } }),
    ]
  ...
  it('publishes a peer before its consumer', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@nuaagent/consumer', { peerDependencies: { '@nuaagent/zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@nuaagent/zebra'),
    ]
  ...
  it('orders around a peer cycle rather than refusing to publish, and reports the edge it dropped', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@nuaagent/left', { peerDependencies: { '@nuaagent/right': 'workspace:^' } }),
      member('packages/a/right', '@nuaagent/right', { peerDependencies: { '@nuaagent/left': 'workspace:^' } }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@nuaagent/right', peer: '@nuaagent/left' },
    ])
  })
  ...
  it('honours an install edge even when a peer cycle surrounds it', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/base', '@nuaagent/base', { peerDependencies: { '@nuaagent/consumer': 'workspace:^' } }),
      member('packages/a/consumer', '@nuaagent/consumer', {
        dependencies: { '@nuaagent/base': 'workspace:^' },
        peerDependencies: { '@nuaagent/base': 'workspace:^' },
      }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@nuaagent/base', peer: '@nuaagent/consumer' },
    ])
  })
  ...
  it('refuses an order that would publish a consumer before a dependency it installs', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@nuaagent/alpha', { peerDependencies: { '@nuaagent/bravo': 'workspace:^' } }),
      member('packages/a/bravo', '@nuaagent/bravo', { peerDependencies: { '@nuaagent/charlie': 'workspace:^' } }),
      member('packages/a/charlie', '@nuaagent/charlie', { dependencies: { '@nuaagent/alpha': 'workspace:^' } }),
    ]
    ...
    expect(() => { dsh.publishOrder(members) }).toThrow(/no publish order honours @nuaagent\/charlie -> @nuaagent\/alpha/)
  })

  it('ignores devDependencies when ordering', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@nuaagent/alpha', { devDependencies: { '@nuaagent/zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@nuaagent/zebra'),
    ]
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    const dsh = releaseFamily('dsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@nuaagent/library')
    const vendored = member('vendor/cordis', '@nuaagent/cordis')
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@nuaagent/cli', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@nuaagent/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@nuaagent/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@nuaagent/library', { files: ['lib/index.js'] }),
