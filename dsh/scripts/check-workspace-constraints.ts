...
const vendoredPackages = new Set([
  ...
  '@nuaagent/cordis-plugin-loader',
  '@nuaagent/cordis-plugin-include',
  '@nuaagent/cordis-plugin-group',
  '@nuaagent/cordis-plugin-timer',
  '@nuaagent/cordis-plugin-hmr',
  '@nuaagent/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@nuaagent/node-addon-landlock-run',
  '@nuaagent/node-addon-landlock-run-linux-arm64',
  '@nuaagent/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@nuaagent/node-addon-landlock-run': ['src/main.c'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@nuaagent/client-ui-primitives': ['lib/**/*.css'],
  '@nuaagent/client-web': ['lib/**/*.css'],
  '@nuaagent/client-ui-theme': ['lib/styles'],
  // The CPython side ships as source .py files, published as-is rather than built.
  '@nuaagent/code-runtime-python': ['py/**/*.py'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@nuaagent/sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@nuaagent/sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  // SQLite loads every statement from immutable package resources at runtime.
  '@nuaagent/session-persistence-sqlite': ['resources/sql/**/*.sql'],
  '@nuaagent/skill-badge': ['assets'],
  '@nuaagent/subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (manifest.name?.startsWith('@nuaagent/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@nuaagent/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@nuaagent/')) {
    const peer = manifest.peerDependencies?.['@nuaagent/cordis']
