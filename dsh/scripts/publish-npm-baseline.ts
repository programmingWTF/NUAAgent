...
const RELEASE_ENTRY_PACKAGE = '@nuaagent/cli'
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@nuaagent/')) {
        throw new Error(`${manifestPath} must name an @nuaagent package`)
      }
      if (name === '@nuaagent/root') {
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@nuaagent/cli/lib/bin.js')
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@nuaagent/') || name === '@nuaagent/root')) {
