 ...
 * `@nuaagent/tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@nuaagent/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@nuaagent/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@nuaagent/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@nuaagent/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@nuaagent/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@nuaagent/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@nuaagent/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@nuaagent/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@nuaagent/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  ...
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@nuaagent/cordis\']', count: 2 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  ...
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@nuaagent/cordis']
    const dev = manifest.devDependencies?.['@nuaagent/cordis']

    if (!peer) errors.push(\`\${label}: @nuaagent/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @nuaagent/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @nuaagent/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
    // The rescoped name is already covered by the `@nuaagent/.+` pattern beside it.
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@nuaagent/\')) {',
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@nuaagent/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: '@nuaagent/cordis-plugin-timer  timer service',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@nuaagent/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@nuaagent/` as "another plugin package".
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@nuaagent/cordis'",
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@nuaagent/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
