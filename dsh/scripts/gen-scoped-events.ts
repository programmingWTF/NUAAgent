...
class ScopedEventGenerator {
  ...
  render(): string {
    ...
    return [
      ...
      ' *',
      ' * @module @nuaagent/scope/scoped-events.generated',
      ' */',
      '',
...
/** Return whether an Events interface is inside declare module '@nuaagent/cordis'. */
function isCordisModuleInterface(node: ts.InterfaceDeclaration): boolean {
  ...
  return ts.isModuleBlock(block)
    ...
    && declaration.name.text === '@nuaagent/cordis'
}
