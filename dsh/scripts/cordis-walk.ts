...
const MERGE_HEAD = /declare module ['"](?:@nuaagent\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@nuaagent/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@nuaagent/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@nuaagent/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@nuaagent/cordis'` Context merge
...
/**
 * Every event name a `declare module '@nuaagent/cordis'` Events merge declares in one
