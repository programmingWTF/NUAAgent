...
import * as GoalSessionInvariant from '@nuaagent/goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@nuaagent/goal-round-driver'
import InvariantRegistry, { InvariantError } from '@nuaagent/invariants'
import SessionStore, { SessionId, type Session } from '@nuaagent/session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@nuaagent/goal-round-driver',
    }))
  })
