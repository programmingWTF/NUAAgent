/**
 * @nuaagent/web-app — the browser-surface bundle's runtime glue plugin
...
import { addHarnessSourceSection } from '@nuaagent/app-boot'
import * as FrontendStatic from '@nuaagent/host-frontend-static'
import type {} from '@nuaagent/cordis-plugin-loader'
...
function webSurfacePrompt(webUrl: string): string {
  ...
  return `You are interacting with the user through the NUAAgent Web GUI at ${webUrl}. `
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('@nuaagent/web-frontend/dist/index.html')
  } catch {
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [DSH_WEB_URL]: { description: 'Canonical local URL of the NUAAgent Web GUI serving this session.' },
        },
