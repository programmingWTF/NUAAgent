...
import type { Scope } from '@nuaagent/scope'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@nuaagent/tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@nuaagent/tools'
import type { Agent } from '@nuaagent/agent'
import { Session, SessionId } from '@nuaagent/session'
import type { JsonValue, SessionEventMap } from '@nuaagent/session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@nuaagent/scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
