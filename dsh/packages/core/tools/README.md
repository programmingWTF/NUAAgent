...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@nuaagent/tool-call-timeout-policy` wrapper.
