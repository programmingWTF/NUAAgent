 ...
 * execution pipeline.
 * @module @nuaagent/tools
 */

import { Context, Service } from '@nuaagent/cordis'
import z from '@nuaagent/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@nuaagent/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@nuaagent/scope'
import type { CallId, ContentBlock, ToolSchema } from '@nuaagent/llm'
import { assertNever, deepFreeze, HarnessError } from '@nuaagent/llm'
import type { Agent } from '@nuaagent/agent'
import { snapshotJsonValue } from '@nuaagent/session'
import type { JsonValue, UserMessage } from '@nuaagent/session'
import type { ToolProviderResult } from '@nuaagent/system-prompt'
import type { CodeRuntime } from '@nuaagent/code-runtime'
...
export type { JsonValue } from '@nuaagent/session'
...
// lives in its own UI-facing module; re-export it so `@nuaagent/tools`
...
declare module '@nuaagent/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@nuaagent/scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@nuaagent/scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@nuaagent/scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@nuaagent/tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@nuaagent/tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @nuaagent/code-runtime-worker-thread) or set tools mode to "native"`)
    }
