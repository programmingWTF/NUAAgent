...
import LlmRuntime, { createUserMessage, CallId, LlmError, MessageSource, ProviderRequestId, StreamChunk  } from '@nuaagent/llm'
import SessionStore, { Session, SessionEvent, SessionId, TurnEndReason, type UserMessage } from '@nuaagent/session'
import SystemPrompt from '@nuaagent/system-prompt'
import ToolRuntime, { defineContentToolFixture, type PostToolDecision } from '@nuaagent/tools'
...
describe('turn and step boundary recovery', () => {
  ...
  it('a step/end validation failure surfaces the resulting open-step invariant', async () => {
    ...
    expect(errors.map(error => error.message)).toEqual([
      ...
      'invariant violated by "@nuaagent/session": turn/end 1 while step 1 is still open',
    ])
    expect(boundaryCounts(agent)).toMatchObject({
      turnStart: 1,
      turnEnd: 0,
