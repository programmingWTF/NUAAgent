...
import SystemPrompt, { AssembleContext, PromptAssembly, renderContextSnapshot, renderPrompt } from '@nuaagent/system-prompt'
...
const IDENTITY = 'You are an AI agent powered by NUAAgent.'
...
describe('SystemPrompt', () => {
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      ...
      await ctx.plugin(SystemPrompt, { persona: 'You are NUAAgent.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are NUAAgent.', 'Be precise.', 'cwd: /tmp'])
    expect(assembly.contexts).toEqual([
