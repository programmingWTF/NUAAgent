 ...
 *
 * @module @nuaagent/system-prompt
 */

import { Context, Service } from '@nuaagent/cordis'
import z from '@nuaagent/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@nuaagent/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@nuaagent/scope'
import type { ContextSnapshotSection, ToolSchema } from '@nuaagent/llm'
...
declare module '@nuaagent/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@nuaagent/scope`): scoped listeners
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by NUAAgent.',
      })
    }
    this.section({
