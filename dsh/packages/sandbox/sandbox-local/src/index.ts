...
} from '@nuaagent/node-addon-landlock-run'
import { Context } from '@nuaagent/cordis'
import z from '@nuaagent/schemastery'
import { assertNever } from '@nuaagent/llm'
import { SandboxProvider, SandboxUnavailableError } from '@nuaagent/sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@nuaagent/sandbox'
import type { SessionId } from '@nuaagent/session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@nuaagent/sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@nuaagent/sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@nuaagent/sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@nuaagent/sandbox-windows-acl/src/runner.ts'))
