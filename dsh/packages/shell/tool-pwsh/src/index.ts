 ...
 * `@nuaagent/pwsh-local`) backs `ctx.shell`; the tool contract is
 ...
 * exit-status parse from `@nuaagent/shell`.
 *
 * @module @nuaagent/tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@nuaagent/cordis'
import z from '@nuaagent/schemastery'
import { defineTool, TOOL_ABORTED } from '@nuaagent/tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@nuaagent/tools'
...
import type {} from '@nuaagent/shell-env'
import type {} from '@nuaagent/user-approval'
import type { SandboxExecutionPolicy, SandboxMode } from '@nuaagent/sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@nuaagent/sandbox'
import type { SandboxPolicyService } from '@nuaagent/sandbox-policy'
import type { ShellRunResult } from '@nuaagent/shell'
import { parseExitStatus } from '@nuaagent/shell'
import { processOutcome } from './background.ts'
import { renderPwshProcessRead, renderPwshResult } from './render.ts'
import type { RenderablePwshResult } from './render.ts'

declare module '@nuaagent/jobs' {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: PwshToolArgs, exec) {
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @nuaagent/jobs and @nuaagent/tool-jobs')
        }
