...
import { ShellExecutor } from '@nuaagent/shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from '@nuaagent/shell'
import SystemPrompt from '@nuaagent/system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@nuaagent/tools'
...
import ApprovalService from '@nuaagent/user-approval'
import type { ApprovalOutcome } from '@nuaagent/user-approval'
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @nuaagent/jobs and @nuaagent/tool-jobs')
  })
