...
| 包 | 职责 |
|---|---|
| `@nuaagent/shell`（本包） | Service Definition：抽象服务 + 词汇类型 |
| `@nuaagent/bash-local` | Service Provider：本地子进程 |
| `@nuaagent/bash-sandbox` | Service Provider：沿用 `dsh-bash-local` 的机制，但通过 [`ctx.sandbox`](../../sandbox/sandbox/) 限制每次 spawn，并将拒绝报告为结果事实 |
| `@nuaagent/tool-bash` | 基于 `ctx.shell`、面向模型的工具 schema |
