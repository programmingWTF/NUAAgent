...
| Package | Role |
|---|---|
| `@nuaagent/shell` (this) | Service Definition: abstract service + vocabulary types |
| `@nuaagent/bash-local` | Service Provider: local subprocesses |
| `@nuaagent/bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@nuaagent/tool-bash` | the model-facing tool schemas over `ctx.shell` |
