...
} from '@nuaagent/typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@nuaagent/typert-protocol'
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@nuaagent/tools', 'ToolInput')).toBe('@nuaagent/tools#ToolInput')
    expect(typertPackageKey('@nuaagent/tools', 'host')).toBe('@nuaagent/tools#host')
    expect(ctx.typert.get('@nuaagent/tools#ToolInput')).toMatchObject({
      package: '@nuaagent/tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@nuaagent/tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@nuaagent/tools', 'host')).toMatchObject({
      key: '@nuaagent/tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@nuaagent/tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@nuaagent/tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@nuaagent/tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@nuaagent/tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@nuaagent/tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@nuaagent/tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@nuaagent/tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@nuaagent/tools#ToolInput')).not.toBe(projected)
  })
