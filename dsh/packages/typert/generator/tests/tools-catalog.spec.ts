...
import type { TypertContribution } from '@nuaagent/typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@nuaagent/tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    ...
    const artifact = new FaceModelEmitter(host).emit('@nuaagent/tools')
    ...
    const record = ctx.typert.getPackage('@nuaagent/tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@nuaagent/tools', 'host')).toBeUndefined()
  })
})
