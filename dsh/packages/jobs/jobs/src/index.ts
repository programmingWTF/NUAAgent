 ...
 * process-local registry lives in `@nuaagent/jobs-local`.
 * @module @nuaagent/jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@nuaagent/jobs is the abstract job registry seam; load an implementation such as @nuaagent/jobs-local instead')
    }
    super(ctx, 'jobs')
  }
