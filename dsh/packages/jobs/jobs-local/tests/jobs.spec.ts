...
import { bindScopeParent, createScope, scopeOf } from '@nuaagent/scope'
import type { ScopeKey } from '@nuaagent/scope'
import { JobId } from '@nuaagent/jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@nuaagent/jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @nuaagent/tool-jobs in its composition)')
  })
