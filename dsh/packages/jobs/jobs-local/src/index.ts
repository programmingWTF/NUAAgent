 ...
 * @module @nuaagent/jobs-local
 */

import { Context } from '@nuaagent/cordis'
import z from '@nuaagent/schemastery'
import type { Agent } from '@nuaagent/agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@nuaagent/scope'
import type { ScopeLayer } from '@nuaagent/scope'
import { deadline, timeoutOf } from '@nuaagent/timeout'
import { JobRegistry, JobId } from '@nuaagent/jobs'
import type {
 ...
 * `@nuaagent/jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @nuaagent/tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @nuaagent/agent)')
    }
