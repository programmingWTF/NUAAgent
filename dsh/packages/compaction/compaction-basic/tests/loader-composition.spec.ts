...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@nuaagent/llm', LlmRuntime],
    ['@nuaagent/session', SessionStore],
    ['@nuaagent/token-meter', TokenMeter],
    ['@nuaagent/compaction-tool-result-pruner', ToolResultPruner],
    ['@nuaagent/compaction-basic', BasicCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-basic YAML order', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@nuaagent/token-meter'",
      "- name: '@nuaagent/compaction-tool-result-pruner'",
      '  config:',
      '    thresholdChars: 100',
      '    headChars: 20',
      '    tailChars: 10',
      "- name: '@nuaagent/compaction-basic'",
      '  config:',
      '    thresholdRatio: 0.5',
      '    retainRatio: 0.125',
      '    auto: false',
