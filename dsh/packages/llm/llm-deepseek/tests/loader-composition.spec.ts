...
import LocalCredentialProvider from '@nuaagent/credentials-local'
import { settingsNamespace } from '@nuaagent/settings'
import FileSettingsProvider from '@nuaagent/settings-file'
...
async function loadComposition(
  ...
  await writeFile(configPath, [
    ...
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@nuaagent/settings-file'",
        '  config:',
        `    path: ${JSON.stringify(settingsPath)}`,
        '    debounceMs: 10',
        '- id: credentials',
        "  name: '@nuaagent/credentials-local'",
        '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@nuaagent/settings-file', FileSettingsProvider],
    ['@nuaagent/credentials-local', LocalCredentialProvider],
    ['@nuaagent/llm-deepseek', LlmDeepSeek],
  ])
  ctx.loader.internal = {
    version: 'v2',
