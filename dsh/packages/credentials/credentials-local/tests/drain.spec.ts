...
import { credentialKey, credentialRef } from '@nuaagent/credentials'
...
vi.mock('@nuaagent/atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@nuaagent/atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@nuaagent/atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
