...
import { renderPrompt, TOOL_ORDER_REST } from '@nuaagent/system-prompt'
import * as agentCore from '../src/index.ts'
import { agentEvents, type Agent } from '@nuaagent/agent'
...
import type { ToolExecution } from '@nuaagent/tools'
...
/**
 * Unit coverage for the @nuaagent/agent-spine-demo bundle: mounting it brings
...
describe('dsh-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@nuaagent/agent$'] },
      { package_blocklist: ['^@nuaagent/session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    ...
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by NUAAgent.')
