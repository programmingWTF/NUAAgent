...
```
@nuaagent/cordis-plugin-timer  timer service (writes nothing to stdout)
@nuaagent/llm              abstract LLM service + content-block vocabulary
@nuaagent/session          event-sourced session log + store
@nuaagent/session-title    log-backed title service + deterministic fallback
@nuaagent/system-prompt    prompt-section + tool-schema assembly
@nuaagent/tools            registry + guarded pre/around/post/final-result pipeline
@nuaagent/skill            skill provider registry
@nuaagent/skill-filesystem      local filesystem skill provider
@nuaagent/agent            agent registry + initiator scope + agent/* events
@nuaagent/goal             optional persisted same-session goal domain
@nuaagent/tool-goal        optional model-facing goal controls
@nuaagent/goal-round-driver     optional same-session goal-round driver
@nuaagent/llm-retry        provider-routed request retry policy
@nuaagent/jobs-local      generic background-job registry
@nuaagent/invariants       configurable invariant registry service
@nuaagent/session/invariant
@nuaagent/agent/invariant
@nuaagent/scope/invariant
@nuaagent/agent-loop/invariant
                                  package-owned relational checks
@nuaagent/tool-bash        the model-facing bash schema (unless toolBash=false)
@nuaagent/agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@nuaagent/tool-skill       session-prefix skill catalog + model-facing loader schema
@nuaagent/tool-jobs       job_output/job_list/job_kill schemas + completion notices
@nuaagent/agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@nuaagent/agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@nuaagent/'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.zh.md)。
