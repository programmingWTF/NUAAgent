...
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@nuaagent/llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@nuaagent/session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@nuaagent/agent'
import AgentLoop from '@nuaagent/agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@nuaagent/fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import ToolRuntime, { defineContentToolFixture } from '@nuaagent/tools'
import type {
  ToolExecution,
  ToolExecutionToken,
} from '@nuaagent/tools'
...
} from '@nuaagent/agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    try {
      ...
      const isolated = await import('@nuaagent/agent-instructions')
