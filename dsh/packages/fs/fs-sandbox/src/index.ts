 ...
 * `@nuaagent/fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@nuaagent/bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@nuaagent/tool-fs`),
 * exactly as bash's does.
 *
 * @module @nuaagent/fs-sandbox
 */

import { Context } from '@nuaagent/cordis'
import { LocalFileSystem } from '@nuaagent/fs-local'
import type { Config as LocalConfig } from '@nuaagent/fs-local'
import { FsError } from '@nuaagent/fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@nuaagent/fs'
import { writableRoots } from '@nuaagent/sandbox'
import type { SandboxExecutionPolicy, SandboxMode } from '@nuaagent/sandbox'
import type {} from '@nuaagent/sandbox-policy'
