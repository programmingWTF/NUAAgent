 ...
 *
 * @module @nuaagent/tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@nuaagent/output-retention'
import type { RetainedItems } from '@nuaagent/output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@nuaagent/subprocess'
 ...
 * `@nuaagent/tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@nuaagent/tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@nuaagent/tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@nuaagent/tool-fs-search/presentation} `grepSearchMeta`)
