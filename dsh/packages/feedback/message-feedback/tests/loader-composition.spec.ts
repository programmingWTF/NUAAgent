...
import Storage from '@nuaagent/storage'
import * as StorageDomain from '@nuaagent/storage-domain'
import * as StorageJson from '@nuaagent/storage-json'
import { remoteMethods } from '@nuaagent/typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@nuaagent/session-persistence-jsonl', JsonlSessionPersistence],
    ['@nuaagent/storage', Storage],
    ['@nuaagent/storage-json', StorageJson],
    ['@nuaagent/storage-domain', StorageDomain],
    ['@nuaagent/message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@nuaagent/session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@nuaagent/storage'",
      "- name: '@nuaagent/storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@nuaagent/storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@nuaagent/message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
