...
| Package | Role |
|---|---|
| `@nuaagent/web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@nuaagent/web-search-exa` | Search provider: Exa |
| `@nuaagent/web-search-perplexity` | Search provider: Perplexity |
| `@nuaagent/web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@nuaagent/tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
