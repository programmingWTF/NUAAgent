...
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@nuaagent/tools'
import { assembleContextFor, type Agent } from '@nuaagent/agent'
...
import type { SubagentStartRequest } from '@nuaagent/subagent'
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @nuaagent/jobs')
  })
