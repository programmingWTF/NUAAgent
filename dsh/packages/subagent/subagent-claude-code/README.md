...
The standalone composition below shows the complete explicit capability. A Profile based on `@nuaagent/base` keeps its existing Job rows, adds the product provider and tool rows, and does not mount duplicate Job services.

```yaml
- id: subagent-claude-safe
  name: '@nuaagent/subagent-claude-code'
  config:
...
```yaml
- id: jobs
  name: '@nuaagent/jobs-local'

- id: tool-jobs
  name: '@nuaagent/tool-jobs'
