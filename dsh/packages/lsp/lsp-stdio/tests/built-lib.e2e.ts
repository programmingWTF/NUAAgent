...
/**
 * Keyless built-artifact smoke: plain Node imports `@nuaagent/lsp` and
 * `@nuaagent/lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@nuaagent/cordis')
      const { default: Lsp } = await import('@nuaagent/lsp')
      const LspLocal = await import('@nuaagent/lsp-stdio')
      const { default: LocalFileSystem } = await import('@nuaagent/fs-local')
      const { default: LocalSubprocessRuntime } = await import('@nuaagent/subprocess-local')
