...
| Package | Role |
|---|---|
| `@nuaagent/lsp` (this) | Service Definition: the service, provider registry keyed by branded id + extension mapping, per-query selection, request/result vocabulary, the `LspError` taxonomy |
| `@nuaagent/lsp-stdio` | Service Provider: a generic local backend that registers configured stdio language-server providers |
| `@nuaagent/tool-lsp` | Consumer: the model-facing `lsp` tool over `ctx.lsp` |
