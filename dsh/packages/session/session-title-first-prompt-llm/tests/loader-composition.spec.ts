...
import LlmRuntime, { createUserMessage, LlmAdapter  } from '@nuaagent/llm'
import type { GenerateOptions, StreamChunk } from '@nuaagent/llm'
import SessionStore, { SessionId } from '@nuaagent/session'
import SessionTitleService from '@nuaagent/session-title'
import * as providerPlugin from '@nuaagent/session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    ...
    "- name: '@nuaagent/session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: '@nuaagent/session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@nuaagent/session-title', SessionTitleService],
    ['@nuaagent/session-title-first-prompt-llm', providerPlugin],
  ])
