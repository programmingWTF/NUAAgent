/**
 * OpenTelemetry Service Provider for the NUAAgent telemetry capability.
 *
...
} from '@nuaagent/session-telemetry'
import { APP_IDENTITY } from '@nuaagent/llm'
import { getOrCreateAnonymousUserId } from '@nuaagent/anonymous-user-id'
import {
...
export class OpenTelemetrySessionBackend extends SessionTelemetryBackend {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    const ledger = this.provider.getLogger('@nuaagent/session-telemetry-otel', version)
    const ops = this.provider.getLogger('@nuaagent/session-telemetry-otel/ops', version)
