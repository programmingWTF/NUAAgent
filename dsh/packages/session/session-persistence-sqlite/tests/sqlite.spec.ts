...
import SessionPersistenceSqlite, {
  ...
  SCHEMA_VERSION,
} from '@nuaagent/session-persistence-sqlite'
import {
...
describe('SessionPersistenceSqlite physical packing', () => {
  it('loads from cordis.yml and packs through the assembled service', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@nuaagent/session-persistence-sqlite'",
      '  config:',
    ...
    ctx.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@nuaagent/session') return SessionStore
        if (specifier === '@nuaagent/session-persistence-sqlite') {
