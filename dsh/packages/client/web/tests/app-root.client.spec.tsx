...
afterEach(cleanup)
import { AppRoot } from '@nuaagent/client-web/src/AppRoot.tsx'
import { createLoaderStatusStore, createSignal } from '@nuaagent/client-web/src/loader-status.ts'
...
describe('AppRoot', () => {
  ...
  it('lists failed entries and stays on the loading page', () => {
    ...
    act(() => {
      status.set('@nuaagent/client-ui-layout', 'failed')
      status.set('ok', 'active')
    })
    expect(getByText('Failed to load plugins')).toBeTruthy()
    expect(getByText('@nuaagent/client-ui-layout')).toBeTruthy()
    expect(queryByTestId('real-ui')).toBeNull()
  })
