...
describe('BootPage', () => {
  ...
  it('lists failed entries', () => {
    const { el, page } = mount()
    page.setState('@nuaagent/client-ui-layout', 'failed')
    page.setState('ok', 'active')
    page.setState('@nuaagent/client-ui-tool', 'failed')
    expect(el.textContent).toContain('@nuaagent/client-ui-layout')
    expect(el.textContent).toContain('@nuaagent/client-ui-tool')
