...
import { renderIndexInjections, type WebServer, type WebRoute } from '@nuaagent/host-webserver'
...
const MODULES_ID = '@nuaagent/client-modules'
const RUNTIME_ID = '@nuaagent/client-runtime'
...
describe('module graph order', () => {
  ...
  it('leaves a request no row answers to the static assembly channel', () => {
    expect(ids(orderByModuleGraph([
      entry('consumer', { external: ['@nuaagent/cordis'] }),
      entry('other'),
