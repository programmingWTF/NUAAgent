 ...
 * Everything else under @nuaagent/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@nuaagent\/(host-apiproxy|file-reference|session|llm|tools|brand)(\/|$)/
...
const GENERATED_REMOTE = /^@nuaagent\/[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @nuaagent value import is a build error — a
      ...
      resolveId(source: string) {
        if (!source.startsWith('@nuaagent/')) return null
