...
Packages here are named with the directory prefix: `@nuaagent/client-<name>`.
...
2. **A dynamic package declares internal dynamic relationships as peer plus dev.** Production source imports, re-exports, module augmentations, and type-only references to an `@nuaagent/*` package count, as does a package named by `dsh.client.inject`. A test-only internal dependency stays dev-only.
