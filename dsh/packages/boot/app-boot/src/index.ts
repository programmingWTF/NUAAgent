...
import { Context, type FiberState } from '@nuaagent/cordis'
import Loader, { type Entry, type EntryOptions } from '@nuaagent/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@nuaagent/cordis-plugin-include'
import Group from '@nuaagent/cordis-plugin-group'
import { dshHomePath, resolveDshHome } from '@nuaagent/home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@nuaagent/launch-environment'
import type {} from '@nuaagent/cordis-plugin-hmr'
 ...
 * entries (`@nuaagent/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@nuaagent/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@nuaagent/cordis-plugin-group`
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The NUAAgent implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend DSH itself.`,
  })
}
