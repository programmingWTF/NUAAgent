...
|---|---|---|
| [`pty`](terminal/README.md) (`@nuaagent/terminal`) | Backend registry, branded ids, exact-Agent ownership, session operations, and awaited cleanup | `ctx.terminals` |
| `terminal-bash` (`@nuaagent/terminal-bash`) | Shell backend over `ctx.subprocess.spawnTerminal`: readiness detection, bounded terminal state, sandbox policy, and session operations | registers on `ctx.terminals` |
| `tool-terminal` (`@nuaagent/tool-terminal`) | Six model-facing tools and generic task integration for background sends | registers on `ctx.tools` |
