...
由模型挂载的动态包在 host 侧的那一半：定义注册表、host 半所用的 `node:vm` 沙箱与 fiber 生命周期、invoke handler 表，以及由某个浏览器页面执行的 run 往返。以 `ctx.dynamicCordisRunner` 提供。面向模型的工具在 [`@nuaagent/tool-cordis`](../tool-cordis/README.md) 中；浏览器半由 [`@nuaagent/cordis-client-runner`](../cordis-client-runner/README.md) 装载。
