...
NUAAgent 用户数据的共享文件系统路径辅助工具。

## DSH 主目录

`resolveDshHome()` 解析 NUAAgent 的单根主目录。优先级从高到低为：显式配置的路径、`$DSH_HOME`、`~/.dsh`。harness 将所有用户数据保存在同一根目录下。
...
`defaultDshHome()` 使用 Node 的平台路径规则，将操作系统主目录与 `.dsh` 拼接，并返回默认 NUAAgent 主目录。
