...
`resolveDshHome()` resolves the single-root NUAAgent home. Precedence, highest first: an explicit configured path, `$DSH_HOME`, then `~/.dsh`. The harness keeps all user data under one root.
...
`defaultDshHome()` returns the default NUAAgent home by joining the operating-system home directory with `.dsh`, using Node's platform path rules.
