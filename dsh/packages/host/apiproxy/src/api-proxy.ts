...
import { installModelSelection } from '@nuaagent/agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@nuaagent/agent'
import type {} from '@nuaagent/agent-presets/types'
import { AttachmentError, admitEncodedImages } from '@nuaagent/attachment'
import type { ImageAttachmentRef } from '@nuaagent/attachment'
import { createUserMessage, freezeMessage, ReasoningEffortId } from '@nuaagent/llm'
import { errorChain } from '@nuaagent/llm'
import type { ContentBlock, MessageSource } from '@nuaagent/llm'
import { isAppendSurfaceEvent, isJsonValue } from '@nuaagent/session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@nuaagent/session'
import type { SessionPersistence } from '@nuaagent/session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@nuaagent/session-query'
import { SubagentError } from '@nuaagent/subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@nuaagent/subagent'
import { isUserInvocable } from '@nuaagent/skill'
import type { Workspace, WorkspaceRecord } from '@nuaagent/workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@nuaagent/workspace'
// Type-only: brings the `ctx.tools` Context merge into this program (viewFor reads presenters).
import {
  InvalidPresetIdError, PresetExistsError, PresetMountError,
  PresetNotWritableError, resolveSessionPreset, UnknownPresetError,
} from '@nuaagent/agent-presets'
import type { PresetBearingSession } from '@nuaagent/agent-presets'
import type {} from '@nuaagent/tools'
import type {
...
import type { SessionRawArtifact } from '@nuaagent/session-persistence'
import {
...
import type {} from '@nuaagent/jobs'
import type { JobSnapshot } from '@nuaagent/jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@nuaagent/session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@nuaagent/goal'
import type { GoalRef as CoreGoalRef } from '@nuaagent/goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@nuaagent/commands'
...
import type {} from '@nuaagent/cordis-host-runner/types'
import type {} from '@nuaagent/skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@nuaagent/settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@nuaagent/settings'
import { credentialRef } from '@nuaagent/credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@nuaagent/session-title'
import type { CallId } from '@nuaagent/llm/brand'
import type { ScopeKey } from '@nuaagent/scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@nuaagent/user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@nuaagent/user-approval'
...
} from '@nuaagent/user-questions'
import { UserQuestionError } from '@nuaagent/user-questions'
import { DirectoryPickerError } from '@nuaagent/host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @nuaagent/session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @nuaagent/goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @nuaagent/settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @nuaagent/credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @nuaagent/session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @nuaagent/skill', details: {} })
        }
