...
import LlmRuntime, { LlmAdapter } from '@nuaagent/llm'
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@nuaagent/llm'
import { SettingsProvider, settingsNamespace } from '@nuaagent/settings'
import type { SettingsNamespace } from '@nuaagent/settings'
import { CredentialProvider } from '@nuaagent/credentials'
import type {
  CredentialInfo,
  CredentialKey,
  CredentialRecord,
  ...
  CredentialRef,
  ResolvedCredential,
} from '@nuaagent/credentials'
import type { HostFrame } from '../src/api/index.ts'
import type { RpcRequest, RpcResponse } from '../src/api/rpc.ts'
import { RpcId } from '../src/api/rpc.ts'
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@nuaagent/agent-default-model'
...
describe('settings domain', () => {
  it('reports an actionable error when no settings provider is mounted', async () => {
    ...
    expect(error.message).toContain('settings-file')
  })
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('credentials-local')
  })
