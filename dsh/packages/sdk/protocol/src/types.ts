/**
 * Named wire types for the NUAAgent SDK runtime protocol: the three
 * request/result pairs and the four server-to-client notification payloads
 * exchanged over the newline-delimited JSON-RPC stdio transport. The server
 * plugin (`@nuaagent/sdk-jsonrpc-server`) and SDK clients share these shapes;
 * `serverInfo.name` stays the wire-stable `deepseek-harness-sdk-runtime`.
 *
 * @module @nuaagent/sdk-protocol/types
 */

import type { ContentBlock } from '@nuaagent/llm'
import type { SessionEvent } from '@nuaagent/session'
import type { SubagentStopReason } from '@nuaagent/subagent'
