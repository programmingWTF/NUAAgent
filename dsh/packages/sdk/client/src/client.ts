/**
 * Low-level JSON-RPC client for a NUAAgent SDK runtime subprocess.
 * {@link HarnessClient} owns the child process: it spawns the runtime, speaks
 * the `@nuaagent/sdk-protocol` wire over the child's stdio, fans
 ...
 *
 * @module @nuaagent/sdk-client/client
 */
...
/**
 * JSON-RPC client for the NUAAgent SDK runtime over subprocess stdio.
 *
...
export class HarnessClient {
   ...
   */
  start(): void {
    if (this.closeTask !== undefined) throw new TransportClosedError('NUAAgent runtime client is closed')
    ...
    child.once('error', (error) => {
      ...
      this.failSubscriptions(this.closedError('NUAAgent runtime failed to start'))
    })
    ...
    child.once('exit', (code) => {
      ...
      settled.exited = true
      maybeSettle()
      this.failSubscriptions(this.closedError('NUAAgent runtime exited'))
    })
    child.once('close', () => {
  ...
  async request(method: string, params?: object, timeoutMs?: number): Promise<unknown> {
    ...
    if (this.exitCode !== undefined || this.spawnError !== undefined) {
      ...
      throw this.closedError('NUAAgent runtime is not running')
    }
    const transport = this.transport
    /* v8 ignore next -- start() either sets the transport or throws */
    if (transport === undefined) throw new TransportClosedError('NUAAgent runtime is not running')
    ...
    try {
      ...
      const timer = setTimeout(() => {
        abandon.abort(new RequestTimeoutError(`${method} timed out after ${timeout}ms waiting for the NUAAgent runtime`))
      }, timeout)
      try {
  ...
  subscribe(filter?: NotificationFilter): NotificationSubscription {
    ...
    if (this.closeTask !== undefined || this.exitCode !== undefined || this.spawnError !== undefined) {
      subscription.fail(this.closedError('NUAAgent runtime closed'))
      return subscription
    }
  ...
  private async performClose(): Promise<void> {
    ...
    this.transport?.close()
    this.failSubscriptions(this.closedError('NUAAgent runtime closed'))
  }
