 ...
 *
 * @module @nuaagent/sdk-jsonrpc-server/server
 */

import type { Context } from '@nuaagent/cordis'
import { resolve } from 'node:path'
import type { Agent, AgentHandle } from '@nuaagent/agent'
import { createUserMessage } from '@nuaagent/llm'
import { carrierKeyOf, type Scoped } from '@nuaagent/scope'
import { SessionId } from '@nuaagent/session'
import type SubagentRuntime from '@nuaagent/subagent'
import type { SubagentRunEndInfo } from '@nuaagent/subagent'
...
export class HarnessSdkJsonRpcServer {
  ...
  async handleRequest(method: string, params: Record<string, unknown> | undefined): Promise<unknown> {
    switch (method) {
      ...
      default:
        throw new Error(`unknown NUAAgent SDK runtime method: ${method}`)
    }
  }
  ...
  private async createSession(sessionId: string): Promise<SessionRecord> {
    ...
    // (@nuaagent/agent-presets README, "Composing a child agent").
