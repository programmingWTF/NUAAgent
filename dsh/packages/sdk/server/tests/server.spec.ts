...
import AgentRegistry, { type Agent, type AgentHandle } from '@nuaagent/agent'
...
import SubagentRuntime, { type SubagentResult, type SubagentRunEndInfo } from '@nuaagent/subagent'
import type { JsonRpcTransportPeer } from '@nuaagent/sdk-protocol'
  ...
  it('rejects unknown JSON-RPC runtime methods', async () => {
    ...
    try {
      ...
      await expect(server.handleRequest('does/not/exist', {}))
        .rejects
        .toThrow('unknown NUAAgent SDK runtime method: does/not/exist')
