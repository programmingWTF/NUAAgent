 ...
 */
export function revealArgv(platform, abs) {
    if (platform === 'win32')
...
export function openArgv(platform, abs) {
    if (platform === 'win32')
        return ['cmd.exe', '/c', 'start', '', abs];
    if (platform === 'darwin')
        return ['open', abs];
    return ['xdg-open', abs];
}
 ...
 */
export function ifNoneMatchSaidFresh(header, etag) {
    if (header === undefined)
        return false;
 ...
 */
export function registerPanelRoutes(ctx, fs, git) {
    ...
    const pollGit = async () => {
        if (!gitProbed) {
            ...
            if (!(await git.gitAvailable())) {
                ...
                for (const subscriber of subscribers)
                    push(subscriber, { kind: 'gitUnavailable' });
            }
        }
        if (gitUnavailable)
            return;
        await Promise.all([...subscribers].map(async (subscriber) => {
            try {
                ...
                if (!(await git.isRepositoryCanonical(subscriber.root)))
                    return;
    ...
    const sse = async (req, res) => {
        ...
        const subscriber = { root: gated.canonical, lastGit: '', res };
