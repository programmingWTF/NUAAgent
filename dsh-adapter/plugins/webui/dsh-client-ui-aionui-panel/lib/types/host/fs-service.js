...
import { isPathInside } from "./gate.js";
/** Production watcher spawn over node:fs. */
const defaultSpawnWatcher = (path, options, listener) => watchPath(path, options, listener);
...
async function resolveInsideRoot(root, rel) {
    if (rel.includes('\0'))
        return { ok: false, error: { code: 'path-outside-root', message: 'invalid path' } };
...
export function probeImageSize(data) {
    try {
        ...
        if (data.length >= 30 && data[8] === 0x57 && data[9] === 0x45 && data[10] === 0x42 && data[11] === 0x50
...
export class FsService {
    ...
    async list(root, rel) {
        ...
        for (const entry of dirents) {
            if (entry.isDirectory() && TREE_SKIP_DIRS.has(entry.name))
                continue;
    ...
    async read(root, rel, asImage) {
        ...
        if (info.isDirectory())
            return { code: 'is-directory', message: `${rel} is a directory` };
        if (asImage) {
    ...
    async readRaw(root, rel) {
        ...
        if (!gated.ok)
            return gated.error;
        if (isGitPath(rel))
            return { code: 'path-outside-root', message: 'refusing to read .git' };
        const resolved = await resolveInsideRoot(gated.canonical, rel);
        if (!resolved.ok)
            return resolved.error;
        let info;
    ...
    async rename(root, rel, newName) {
        ...
        if (!gated.ok)
            return gated.error;
        if (rel === '')
            return { code: 'path-outside-root', message: 'refusing to rename the root' };
        if (isGitPath(rel))
            return { code: 'path-outside-root', message: 'refusing to touch .git' };
    ...
    async mkdir(root, rel) {
        ...
        if (!gated.ok)
            return gated.error;
        if (rel === '')
            return { code: 'path-outside-root', message: 'refusing to create the root' };
        if (isGitPath(rel))
            return { code: 'path-outside-root', message: 'refusing to touch .git' };
        const resolved = await resolveInsideRoot(gated.canonical, rel);
        if (!resolved.ok)
    ...
    async delete(root, rel) {
        ...
        if (!gated.ok)
            return gated.error;
        if (rel === '')
            return { code: 'path-outside-root', message: 'refusing to delete the root' };
        if (isGitPath(rel))
            return { code: 'path-outside-root', message: 'refusing to touch .git' };
        const resolved = await resolveInsideRoot(gated.canonical, rel);
        if (!resolved.ok)
        ...
        void this.gate(root).then((gated) => {
            ...
            try {
                watcher = this.spawnWatcher(gated.canonical, { recursive: true }, (_event, filename) => {
                    ...
                    if (name !== null && isIgnoredWatchPath(name))
                        return;
                    fire();
                });
                watcher.on('error', () => {
