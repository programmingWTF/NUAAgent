...
const TOOL_ANALYSIS_TAG_BLOCKS = /<(commit_analysis|context|function_analysis|pr_analysis)>.*?<\/\1>\n?/gs;
/**
 ...
 */
const ISSUE_REFERENCE_PATTERN = /(^|[^\w./-])([A-Za-z0-9][\w-]*\/[A-Za-z0-9][\w.-]*)#(\d+)\b/g;
/**
...
function dispatch(token, state) {
    if (isToken(token, 'blockquote'))
        return renderBlockquote(token, state);
    if (isToken(token, 'code'))
        return renderCodeBlock(token, state);
    if (isToken(token, 'codespan'))
        return paintInlineCode(token.text);
    if (isToken(token, 'em'))
        return renderEmphasis(token, state);
    if (isToken(token, 'strong'))
        return renderStrong(token, state);
    if (isToken(token, 'heading'))
        return renderHeading(token, state);
    if (isToken(token, 'hr'))
        return '---';
    if (isToken(token, 'image'))
        return token.href;
    if (isToken(token, 'link'))
        return renderLink(token, state);
    if (isToken(token, 'list'))
        return renderList(token, state);
    if (isToken(token, 'list_item'))
        return renderListItem(token, state);
    if (isToken(token, 'paragraph'))
        return renderParagraph(token, state);
    if (isToken(token, 'space') || isToken(token, 'br'))
        return EOL;
    if (isToken(token, 'text'))
        return renderText(token, state);
    if (isToken(token, 'table'))
        return renderTable(token, state);
    if (isToken(token, 'escape'))
        return token.text;
...
function renderBlockquote(token, state) {
...
function renderCodeBlock(token, state) {
    if (!state.highlight) {
...
function renderLink(token, state) {
...
function renderList(token, state) {
...
function renderListItem(token, state) {
    const indent = '  '.repeat(state.listDepth);
    const childState = withParent({ ...state, listDepth: state.listDepth + 1 }, token);
    return token.tokens.map(child => indent + dispatch(child, childState)).join('');
}
function renderParagraph(token, state) {
    return token.tokens.map(child => dispatch(child, fresh(state))).join('') + EOL;
}
function renderText(token, state) {
...
function renderTable(token, state) {
...
function cellDisplayText(cell, state) {
    return stripAnsi(cell.tokens.map(child => dispatch(child, fresh(state))).join(''));
}
function renderTableRow(cells, columnWidths, aligns, state) {
    let line = '| ';
    cells.forEach((cell, index) => {
        const content = cell.tokens.map(child => dispatch(child, fresh(state))).join('');
        line +=
            padAligned(content, stringWidth(cellDisplayText(cell, state)), columnWidths[index], aligns[index]) + ' | ';
    });
...
function linkifyIssueReferences(text) {
    ...
    return text.replace(ISSUE_REFERENCE_PATTERN, (_match, prefix, repo, issueNumber) => prefix +
        createHyperlink(`https://github.com/${repo}/issues/${issueNumber}`, `${repo}#${issueNumber}`));
}
/**
 ...
 */
function formatListMarker(listDepth, ordinal) {
    switch (listDepth) {
        case 2:
