...
import { aggregate, burstErrors, columnOfIndex, inspectNode, projectWave, } from '../dsh-adapter/trajectory/index.js';
...
export function TrajectoryScene({ channel, build, onClose, }) {
    ...
    const [projection, setProjection] = React.useState('compressed');
    ...
    const { rows: filtered, indexes } = React.useMemo(() => applyQuery(nodes, query), 
    ...
    // oxlint-disable-next-line react-hooks/exhaustive-deps
    () => aggregate(build, sort), [build, nodes.length, sort]);
    ...
    const windowStart = Math.max(0, Math.min(clampedCursor - Math.floor(ledgerRows / 2), filtered.length - ledgerRows));
    const band = React.useMemo(
    // oxlint-disable-next-line react-hooks/exhaustive-deps
    () => projectWave(nodes, bandWidth, projection), [nodes, nodes.length, bandWidth, projection]);
    const matchColumns = React.useMemo(() => {
        if (query.empty)
            return undefined;
        const set = new Set();
        for (const index of indexes)
            set.add(columnOfIndex(band, index));
        return set;
    }, [band, indexes, query.empty]);
    const focused = filtered[clampedCursor];
    const detail = React.useMemo(() => (focused === undefined ? undefined : inspectNode(focused, channel.traceEvents())), [focused, channel]);
    // ── navigation helpers ───────────────────────────────────────────────────
    ...
    const seek = React.useCallback((predicate, forward) => {
    ...
    useInput((input, key) => {
        ...
        if (key.rightArrow)
            return switchView('hotspot');
        if (input === 'h')
            return switchView(view === 'hotspot' ? 'timeline' : 'hotspot');
        if (input === '/') {
            setQueryOpen(true);
            return;
        }
        if (view === 'hotspot') {
            ...
            if (key.upArrow)
                return setHotCursor(previous => Math.max(0, previous - 1));
            if (key.downArrow)
                return setHotCursor(previous => Math.min(total - 1, previous + 1));
            if (input === 't') {
        ...
        if (input === '[')
            return seek(isFailure, false);
        if (input === ']')
            return seek(isFailure, true);
        if (input === '{')
            return seek(index => filtered[index]?.kind === 'turn', false);
        if (input === '}')
            return seek(index => filtered[index]?.kind === 'turn', true);
        if (input === 'm') {
    ...
    const totalsText = t('traj-totals', { turns: totals.turns, steps: totals.rows }) +
    ...
    const headerLine = spread(`\u2726 ${t('traj-title')}  ${channel.sessionTitle ?? channel.cwd}`, totalsText, bandWidth);
    const header = (_jsx(Box, { width: "100%", height: 1, flexShrink: 0, children: _jsxs(Text, { children: [_jsx(Text, { color: "claude", bold: true, children: `\u2726 ${t('traj-title')}` }), _jsx(Text, { color: "subtle", children: headerLine.left.slice((`\u2726 ${t('traj-title')}`).length) }), _jsx(Text, { children: headerLine.gap }), _jsx(Text, { color: totals.errors > 0 ? 'error' : 'subtle', children: headerLine.right })] }) }));
    const axisLabel = view === 'hotspot' ? t(`traj-sort-${sort}`) : t(`traj-proj-${projection}`);
    const tabsLeft = `${view === 'timeline' ? '\u25cf' : '\u25cb'} ${t('traj-tab-timeline')}  ` +
        `${view === 'hotspot' ? '\u25cf' : '\u25cb'} ${t('traj-tab-hotspot')}`;
    const queryText_ = queryOpen || !query.empty
    ...
    const tabs = (_jsx(Box, { width: "100%", height: 1, flexShrink: 0, children: _jsxs(Text, { children: [_jsx(Text, { color: view === 'timeline' ? 'permission' : 'subtle', bold: view === 'timeline', children: tabsLine.left.slice(0, tabsLeft.length) }), _jsx(Text, { color: "suggestion", children: tabsLine.left.slice(tabsLeft.length) }), _jsx(Text, { children: tabsLine.gap }), _jsx(Text, { color: "subtle", children: tabsLine.right })] }) }));
    const hints = view === 'hotspot'
        ? t('traj-hint-hotspot')
        : queryOpen
            ? t('traj-hint-query')
            : expanded
    ...
    return (_jsxs(Box, { ref: ref, flexDirection: "column", width: "100%", paddingX: 1, children: [header, tabs, _jsx(WaveBand, { band: band, width: bandWidth, cursorColumn: columnOfIndex(band, indexes[clampedCursor] ?? 0), viewportStart: columnOfIndex(band, indexes[windowStart] ?? 0), viewportEnd: columnOfIndex(band, indexes[Math.min(filtered.length - 1, windowStart + ledgerRows - 1)] ?? 0), matches: matchColumns, tick: tick, alertTick: alertTick }), _jsx(Box, { height: 1, flexShrink: 0, children: _jsx(Text, { children: " " }) }), view === 'timeline' ? (_jsxs(_Fragment, { children: [_jsx(Ledger, { rows: filtered, start: windowStart, height: ledgerRows, cursor: clampedCursor, width: columns - 4, tick: tick, arrivalTick: arrivalTick, arrivalFrom: arrivalFrom }), _jsx(Divider, { color: "permission", width: bandWidth }), _jsx(Inspector, { node: focused, detail: detail, height: inspectorRows, width: columns - 4, expanded: expanded, scroll: inspectScroll })] })) : (_jsx(HotspotView, { agg: agg, sort: sort, width: columns - 4, height: ledgerRows + inspectorRows + 1, cursor: hotCursor, tick: tick, switchTick: switchTick })), _jsx(Box, { height: 1, flexShrink: 0, children: _jsx(Text, { children: " " }) }), _jsx(Box, { width: "100%", height: 1, flexShrink: 0, children: _jsxs(Text, { dimColor: true, italic: true, wrap: "truncate", children: [_jsx(HintLine, { text: hints }), totals.tokens.input > 0 ? (_jsx(Text, { color: "subtle", children: `   ${formatTokens(totals.tokens.input)}→${formatTokens(totals.tokens.output)}` })) : ('')] }) })] }));
}
