...
export function WaveBand({ band, width, cursorColumn, viewportStart, viewportEnd, matches, tick, alertTick, }) {
    ...
    for (let column = 0; column < band.buckets.length; column++) {
        ...
        const level = Math.min(LEVELS, Math.max(1, Math.round(((Math.log1p(bucket.weight) - logFloor) / logSpan) * (LEVELS - 1)) + 1));
        const failed = bucket.error || bucket.retry;
        const base = failed
            ? mix(theme.error, theme.warningShimmer, alertPhase)
            : channelColor(dominantChannel(bucket), theme);
        const colour = dimmed
            ? theme.subtle
            : isCursor
                ? mix(base, theme.permissionShimmer, 0.55)
                : base;
    ...
    return (_jsxs(Box, { flexDirection: "column", flexShrink: 0, children: [_jsx(Text, { children: wave }), _jsx(Text, { children: rulerText })] }));
}
/**
