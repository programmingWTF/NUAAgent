 ...
 */
export function miniWakeWidth(columns) {
    if (columns >= 120)
        return 16;
    if (columns >= 100)
        return 12;
...
export function MiniWake({ band, hint, tick, }) {
    ...
    for (const bucket of band.buckets) {
        ...
        const level = Math.min(BLOCKS.length, Math.max(1, Math.round(((Math.log1p(bucket.weight) - logFloor) / logSpan) * (BLOCKS.length - 1)) + 1));
    ...
    return (_jsxs(Box, { flexShrink: 0, flexDirection: "row", gap: 1, children: [_jsx(Text, { children: strip }), hint !== undefined ? _jsx(Text, { color: "subtle", children: hint }) : null] }));
}
