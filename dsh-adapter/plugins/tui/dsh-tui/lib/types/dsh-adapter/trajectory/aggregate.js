...
const NO_TOKENS = { input: 0, output: 0, think: 0, cacheRead: 0, cacheWrite: 0 };
/**
 ...
 */
export function forEachCall(nodes, visit) {
    for (let index = 0; index < nodes.length; index++) {
        const node = nodes[index];
        if (node.burst !== undefined) {
            for (const member of node.burst.members)
                visit(member, index);
            continue;
        }
        if (node.kind === 'tool' || node.kind === 'subtool')
            visit(node, index);
    }
}
/** Get or create a bucket, remembering the first ledger index that hit it. */
function bucket(map, key, ledgerIndex) {
...
export function sortRows(rows, sort) {
    const key = (row) => sort === 'count' ? row.count : sort === 'tokens' ? row.tokens : row.totalMs;
 ...
 */
export function aggregate(build, sort = 'duration') {
    ...
    const model = [];
    if (decodeMs > 0)
        model.push({ label: 'decode', totalMs: decodeMs, count: ttftSamples, tokens: tokens.output, firstIndex: 0 });
    if (ttftSamples > 0)
        model.push({ label: 'ttft', totalMs: ttftMs, count: ttftSamples, tokens: 0, firstIndex: 0 });
 ...
 */
function modelTiming(timing) {
    let decodeMs = 0;
    let ttftMs = 0;
    let ttftSamples = 0;
    for (const slot of timing.values()) {
        if (slot.firstChunk === undefined)
            continue;
        ttftMs += Math.max(0, slot.firstChunk - slot.startTime);
        ttftSamples += 1;
        if (slot.lastChunk !== undefined)
            decodeMs += Math.max(0, slot.lastChunk - slot.firstChunk);
    }
