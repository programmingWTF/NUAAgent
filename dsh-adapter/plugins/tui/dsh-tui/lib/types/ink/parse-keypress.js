 ...
 */
function win32TextKey(raw, name, sequence = name) {
    return {
        kind: 'key',
        name,
        fn: false,
 ...
 *   record).
 */
function parseWin32KeyEvent(s, ctx) {
    ...
    if (vk === 0) {
        ...
        if (uc === 13 || uc === 10)
            return { key: { ...base, name: 'return', sequence: s }, repeat };
        if (uc === 9)
            return { key: { ...base, name: 'tab', sequence: s }, repeat };
        if (uc === 8 || uc === 0x7f)
            return { key: { ...base, name: 'backspace', sequence: s }, repeat };
        return null;
    }
 ...
 */
function feedWin32Paste(state, key) {
    ...
    if (state.matched > 0) {
        ...
        if (state.active) {
            for (const k of held)
                state.buffer += win32RecordChar(k) ?? '';
...
export function parseMultipleKeypresses(prevState, input = '') {
    ...
    const win32Paste = prevState.win32Paste ?? {
        active: false,
        matched: 0,
        held: [],
        buffer: '',
    ...
    if (isFlush && win32Paste.active) {
        ...
        for (const k of win32Paste.held)
            content += win32RecordChar(k) ?? '';
