...
export const QUERY_FIELDS = ['tool:', 'kind:', 'turn:', 'err:', 'run:', '>10s', '<1s', 'tok>1k'];
...
export function parseQuery(raw) {
    ...
    if (trimmed === '')
        return { raw, terms: [], empty: true };
    const terms = [];
    for (const token of trimmed.split(/\s+/)) {
        const lower = token.toLowerCase();
        if (lower === 'err:' || lower === 'error:') {
            terms.push({ kind: 'error' });
            continue;
        }
        if (lower === 'run:' || lower === 'running:') {
            terms.push({ kind: 'running' });
            continue;
        }
        const tool = /^tool:(.+)$/.exec(lower);
        if (tool !== null) {
            terms.push({ kind: 'tool', value: tool[1] });
            continue;
        }
        const rowKind = /^kind:(.+)$/.exec(lower);
        if (rowKind !== null) {
            terms.push({ kind: 'rowKind', value: rowKind[1] });
            continue;
        }
        const turn = /^turn:(\d+)$/.exec(lower);
        if (turn !== null) {
            terms.push({ kind: 'turn', value: Number(turn[1]) });
            continue;
        }
        ...
        if (tokens !== null) {
            ...
            if (count !== undefined) {
                terms.push({ kind: 'tokens', op: tokens[1], count });
                continue;
            }
        }
        ...
        if (duration !== null) {
            ...
            if (ms !== undefined) {
                terms.push({ kind: 'duration', op: duration[1], ms });
                continue;
            }
        }
 ...
 */
export function applyQuery(nodes, query) {
    if (query.empty)
        return { rows: [...nodes], indexes: nodes.map((_, index) => index) };
    const rows = [];
    const indexes = [];
