 ...
 */
export function ledgerLayout(columns) {
    // Fixed costs: spine (2) + clock (8) + pill (6) + cost (1) + duration (7),
    // plus one cell of gap between each.
    if (columns >= 110)
        return { badge: 6, index: true, outcome: true, detail: Math.max(12, columns - 72) };
    if (columns >= 90)
        return { badge: 6, index: true, outcome: true, detail: Math.max(12, columns - 64) };
    if (columns >= 70)
        return { badge: 6, index: false, outcome: false, detail: Math.max(12, columns - 26) };
